...
namespace Wass.Cli.Services
{
    ...
    public sealed class CommandValidator(IAsset _asset, IOptions<DestinationConfig> _config, IOptions<SecurityConfig> _security) : ICommandValidator
    {
        ...
        {
            var exists = _asset.Exists(file);

            if (exists.IsTrue(Lambda.Identity))
            {
                var size = _asset.Size(file, UnitVariant.GB);
                if (size) return size > 0D && size < 2D; // After 2GB, chucking the file into 2GB reads will be required, upto a maximum of 5GB. So might as well swap to the MultiPartUpload function at that point.
            }
